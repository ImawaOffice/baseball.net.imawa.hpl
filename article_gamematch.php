		...
		<div class="gamematch-content">
			...
			<form method="post" action=""  >
				...
				<div class="maintenanceForm">
					...
					<input type="hidden" id="game_date" name="game_date" value="">
					<input type="hidden" id="game_place" name="game_place" value="">
					<input type="hidden" id="winner_id" name="winner_id" value="">
					<input type="hidden" id="loser_id" name="loser_id" value="">
					<input type="hidden" id="team1_name" name="team1_name" value="">
					<input type="hidden" id="team2_name" name="team2_name" value="">
					<input type="hidden" id="team1_score" name="team1_score" value="">
					<input type="hidden" id="team2_score" name="team2_score" value="">
					...
					<div class=form-group>
						<label for="team1_id">チーム名１</label>
						<select id="team1_id" name="team1_id">
							<?php foreach ( getGameUnsetTeamList( $_SESSION[ 'FROM_TOURNAMENT_ID' ] ) as $team ) : ?>
								<option 
									value="<?php echo htmlspecialchars( $team[ 'team_id' ], ENT_QUOTES, 'UTF-8' ); ?>"
								>
					...
					<div class=form-group>
						<label for="team2_id">チーム名２</label>
				...
				<div class="teamdetail">
					...
					<table class="table" id="gameTable">
						...
						<tbody>
							<?php foreach ( getGameList( $_SESSION[ 'FROM_TOURNAMENT_ID' ], $_SESSION[ 'FORM_PAGE_CURRENT' ], $_SESSION[ 'FORM_PAGE_ROWS' ] ) as $game ) : ?>
								<tr 
									...
									data-trgamedate="<?php echo $game[ 'game_date' ]; ?>"
									data-trgameplace="<?php echo $game[ 'game_place' ]; ?>"
								...
								>
									...
									<td class="text-center"><button type="submit" class="delete_button" name="delete_button" value="<?php echo $game[ 'game_id' ]; ?>"><?php echo "削除"; ?></button></td>
								</tr>
							<?php endforeach; ?>
						</tbody>
					</table>
...
<script>
	...
	document.getElementById( "gameTable" ).addEventListener( "click", ( e ) => {
		...
		game_date.required = false;
		game_place.value = tr_game_place;
		game_place.required = false;
		...
		const select_team1 = document.getElementById( 'team1_id' );

		select_team1.add( new Option( tr_team1_name, tr_team1_id ), select_team1.options[0] );

		team2_id.value = tr_team2_id;
		team2_id.required = true;
								
		const select_team2 = document.getElementById( 'team2_id' );

		select_team2.add( new Option( tr_team2_name, tr_team2_id ), select_team2.options[0] );
