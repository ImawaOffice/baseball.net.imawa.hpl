...
function getMaxGameCount( $p_tournamentId, $p_gameClass, $p_gameBlock ) {
	...
	global $g_Log;
	$g_Log->notice( "試合回数取得処理 tournament_id: $p_tournamentId, game_class: $p_gameClass, game_block: $p_gameBlock", __FUNCTION__, basename( __FILE__ ) );
...
function updateGame(){
	...
	if ( $team1_score > $team2_score ) {
		$winner_id = $team1_id;
		$loser_id = $team2_id;
		$g_Log->notice( "勝者ID: $team1_id( $team1_score ) 敗者ID: $team2_id( $team2_score )", __FUNCTION__, basename( __FILE__ ) );
	}
	...
	if ( $team1_score < $team2_score ) {
		$winner_id = $team2_id;
		$loser_id = $team1_id;
		$g_Log->notice( "勝者ID: $team2_id( $team2_score ) 敗者ID: $team1_id( $team1_score )", __FUNCTION__, basename( __FILE__ ) );
	}
		...
		if ( $winner_id != 0 ) {
			switch( $winner_id ) {
				...
				case $team2_id:
					...
					break;
				default:
					$g_Log->notice( "勝者IDがチームIDと一致しないためスコアから勝者を判定します", __FUNCTION__, basename( __FILE__ ) );
	...
	if( $winner_id != 0 ) {
		$g_Log->notice( "次の試合内容登録を更新します", __FUNCTION__, basename( __FILE__ ) );
	...
	if( $loser_id != 0 ) {
		if( $game_block == 1 ){ // ゲームブロックが1の場合は敗者復活戦のため裏大会を作成
			$g_Log->notice( "次の裏試合内容登録を更新します", __FUNCTION__, basename( __FILE__ ) );
			if ( insertUraGame( $tournament_id, 1, 1, $loser_id ) ) {
				$g_Log->notice( "次の裏試合登録内容を更新しました", __FUNCTION__, basename( __FILE__ ) );
			} else {
				$g_Log->notice( "次の裏試合登録内容の更新に失敗しました", __FUNCTION__, basename( __FILE__ ) );
				$_SESSION[ 'FORM_ERR_MSG' ] = "次の裏試合登録内容の更新に失敗しました。";
				return false;
			}
		}
	}
...
function nextGameMatch( $p_tournamentId, $p_gameClass, int $round, int $matchNo): ?array {
	...
	Global $g_Log;
	$g_Log->notice( "次の試合番号を計算します tournament_id: $p_tournamentId, game_class: $p_gameClass, round: $round, matchNo: $matchNo", __FUNCTION__, basename( __FILE__ ) );
	...
	$g_Log->notice( "試合数からラウンド数を計算します 総試合数: $gameMaxCount, 総ラウンド数: $gameTotalRound, tournament_id: $p_tournamentId, game_class: $p_gameClass, round: $round, matchNo: $matchNo", __FUNCTION__, basename( __FILE__ ) );
	...
	Global $g_Log;
	$g_Log->notice( "次の試合番号を計算しました nextRound: $nextRound, nextMatchNo: $nextMatchNo, slot: $slot", __FUNCTION__, basename( __FILE__ ) );
...
function insertNextGame( $p_tournamentId, $p_gameClass, $p_gameBlock, $p_gameCount, $p_teamId ){
	...
	global $g_Log;
	$g_Log->notice( "次回戦追加処理 tournament_id: $p_tournamentId, game_class: $p_gameClass, game_block: $p_gameBlock, game_count: $p_gameCount, team_id: $p_teamId", __FUNCTION__, basename( __FILE__ ) );
	...
	try {
		$g_Log->notice( "試合で登録済みを検索します。[tournament_id={$tournament_id}, game_class={$game_class}, game_block={$new_game_block}, team_id={$team_id}]", __FUNCTION__, basename( __FILE__ ) );
		
		global $g_DB;
		$dataTable = $g_DB->select( $SQL_Select, $SQL_Select_Parameters );

		$g_Log->notice( "試合検索 : 件数=" . count( $dataTable ) . " 件", __FUNCTION__, basename( __FILE__ ) );

		foreach( $dataTable as $row ){
			$g_Log->notice( "ROW: tournament_id={$row['tournament_id']}, game_class={$row['game_class']}, game_block={$row['game_block']}, game_count={$row['game_count']},team1_id={$row['team1_id']}, team2_id={$row['team2_id']}, team_slot={$row['team_slot']}", __FUNCTION__, basename( __FILE__ ) );
		...
		if( count( $dataTable ) > 0 ){
			$g_Log->notice( "チームID:{$team_id} が登録済みのため処理をスキップします。", __FUNCTION__, basename( __FILE__ ) );
			return true; // 大会が存在する場合は更新成功とする
		}

	} catch ( Exception $e ) {
		$_SESSION[ 'FORM_ERR_MSG' ] = '大会内容の更新に失敗しました';
		$g_Log->error( $_SESSION[ 'FORM_ERR_MSG' ] . " : " . htmlspecialchars( $e->getMessage() ), __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
	...
	if ( $new_game === null ) {
		$g_Log->notice( "次の試合が存在しないため、次の試合登録内容の更新をスキップします", __FUNCTION__, basename( __FILE__ ) );
	...
	try {
		...
		if( $stmt === 0 ){
			...
			$g_Log->notice( "次試合追加処理 件数: $stmt", __FUNCTION__, basename( __FILE__ ) );
...
function insertUraGame( $p_tournamentId, $p_gameClass, $p_gameBlock, $p_teamId ) {
	...
	global $g_Log;
	$g_Log->notice( "裏試合情報登録処理 tournament_id: $p_tournamentId, team_id: $p_teamId, game_class: $p_gameClass, game_block: $p_gameBlock", __FUNCTION__, basename( __FILE__ ) );

	if( $p_gameClass != 1 || $p_gameBlock != 1 ) {
		$g_Log->notice( "ゲームクラスまたはゲームブロックが裏試合の条件を満たさないため、裏試合登録内容の更新をスキップします", __FUNCTION__, basename( __FILE__ ) );
	...
	try {
		$g_Log->notice( "裏試合で登録済みを検索します。[tournament_id={$tournament_id}, game_class={$game_class}, game_block={$game_block}, team_id={$team_id}]", __FUNCTION__, basename( __FILE__ ) );
		
		global $g_DB;
		$dataTable = $g_DB->select( $SQL_Select, $SQL_Select_Parameters );

		$g_Log->notice( "裏試合検索 : 件数=" . count( $dataTable ) . " 件", __FUNCTION__, basename( __FILE__ ) );
		...
		$_SESSION[ 'FORM_ERR_MSG' ] = '裏大会内容の更新に失敗しました';
		$g_Log->error( $_SESSION[ 'FORM_ERR_MSG' ] . " : " . htmlspecialchars( $e->getMessage() ), __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
	...
	try {
		$g_Log->notice( "裏試合で対戦相手の決まっていない試合を検索します。", __FUNCTION__, basename( __FILE__ ) );
		...
		foreach( $dataTable as $row ){
			$g_Log->notice( "ROW: tournament_id={$row['tournament_id']}, game_class={$row['game_class']}, game_block={$row['game_block']}, game_count={$row['game_count']}, team_slot={$row['team_slot']}", __FUNCTION__, basename( __FILE__ ) );
			$game_count = $row[ 'game_count' ];
			$team1_id   = $row[ 'team1_id' ];
			$team2_id   = $row[ 'team2_id' ];
			$team_slot  = $row[ 'team_slot' ];
			$g_Log->notice( "対戦相手が決定していない裏試合情報取得 : クラス:$game_class, ラウンド:$game_block, ゲーム数:$game_count, チームスロット:$team_slot", __FUNCTION__, basename( __FILE__ ) );
		}
		...
		if( count( $dataTable ) === 0 ){
			...
			$g_Log->notice( "INSERT: tournament_id={$tournament_id}, game_class={$game_class}, game_block={$game_block}, game_count=" . ( $max_game_count + 1 ) . ", team1_id={$team_id}, team2_id=0", __FUNCTION__, basename( __FILE__ ) );

			$stmt = $g_DB->execute( $SQL_Insert, $SQL_Insert_Parameters );

			$g_Log->notice( "裏試合を追加しました。", __FUNCTION__, basename( __FILE__ ) );
			...
			$g_Log->notice( "UPDATE: tournament_id={$tournament_id}, game_class={$game_class}, game_block={$game_block}, game_count={$game_count}, team1_id={$team_id}, team2_id=0", __FUNCTION__, basename( __FILE__ ) );

			$stmt = $g_DB->execute( $SQL_Update, $SQL_Update_Parameters );

			$g_Log->notice( "裏試合を更新しました。", __FUNCTION__, basename( __FILE__ ) );
