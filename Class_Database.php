...
class Class_Database extends Class_Log {
	...
	public function openDatabase() {
		...
		try {
			...
			$this->Log->notice( "{$Db_Type} データベースに接続しました： $Db_Host", __FUNCTION__, basename( __FILE__ ) );
	...
	public function closeDatabase() {
		...
		$this->PDO = null;
		$this->Log->notice( "データベース接続を閉じました。", __FUNCTION__, basename( __FILE__ ) );
	...
	public function execute( $p_SQL = "", $p_Parameters = array() ) {
		...
		$this->Log->notice( "SQL : $p_SQL", __FUNCTION__, basename( __FILE__ ) );
		...
		try {
			...
			foreach ( $p_Parameters as $key => $value ) {
				...
				$this->Log->notice( "Bind : $key = $value", __FUNCTION__, basename( __FILE__ ) );
	...
	public function execute2( $p_SQL = "", $p_Parameters = array() ) {
		...
		try {
			...
			$this->Log->notice( "SQL : " . $rowCount . " 件実行しました。新しいID : " . $new_id, __FUNCTION__, basename( __FILE__ ) );
		} catch ( Exception $e ) {
			$this->Log->error( $e->getMessage(), __FUNCTION__, basename( __FILE__ ) );
			return false;
		}
