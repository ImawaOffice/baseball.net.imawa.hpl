...
function initProc() {
	...
	global $g_Log;
	$g_Log->notice( "S : 初期処理", __FUNCTION__, basename( __FILE__ ) );
	
	// サインイン済みの場合はホームへリダイレクト
	if( isAuthenticated() ) {
		$g_Log->notice( "サインイン済みのためホームへリダイレクトします。", __FUNCTION__, basename( __FILE__ ) );
		$g_Log->notice( "E : 初期処理", __FUNCTION__, basename( __FILE__ ) );
		header( "Location: ./" );
		exit();
	}
...
function requestProc() {
	...
	global $g_Log;
	$g_Log->notice( "S : リクエスト処理", __FUNCTION__, basename( __FILE__ ) );
	...
	if ( $_SERVER[ 'REQUEST_METHOD' ] === 'GET' ) {
		...
		$g_Log->notice( "E : リクエスト処理", __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
...
function checkProc() {
	...
	global $g_Log;
	$g_Log->notice( "S : チェック処理", __FUNCTION__, basename( __FILE__ ) );

	// ユーザーコードのチェック
	if( ! checkUserCd( $_POST[ 'usercd' ] ) ) {
		$g_Log->notice( "E : チェック処理", __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
		...
		<div class="signin-container">
			<form method="post" action="./signin" novalidate >
				...
				<div class=form-group>
					...
					<input type="email" id="usercd" name="usercd" value="<?php echo isset( $_SESSION[ 'AUTH_USER_CD' ] ) ? htmlspecialchars( $_SESSION[ 'AUTH_USER_CD' ] ) : ''; ?>" <?php echo $usercd_readonly; ?> <?php echo $usercd_autofocus; ?> required>
				</div>
