...
function checkForm(){
	...
	if( ! checkRequest( 'game_place', 'POST' ) ) {
		$_SESSION[ 'RES_ERR_MSG' ] = "開催場所エラー";
		$g_Log->debug( "開催場所のチェックに失敗しました", __FUNCTION__, basename( __FILE__ ) );
		return false;
	}

	// 開催日
	if( ! checkRequest( 'game_date', 'POST' ) ) {
		$_SESSION[ 'RES_ERR_MSG' ] = "開催日エラー";
		$g_Log->debug( "開催日のチェックに失敗しました", __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
...
function getMaxGameCount( $p_tournamentId, $p_gameBlock ) {
	...
	global $g_Log;
	$g_Log->debug( "試合回数取得処理 tournament_id: $p_tournamentId, game_block: $p_gameBlock", __FUNCTION__, basename( __FILE__ ) );
...
function updateGame(){
	...
	if( $loser_id != 0 ) {
		if( $game_block == 1 ){ // ゲームブロックが1の場合は敗者復活戦のため裏大会を作成
			...
			$g_Log->debug( "裏大会の取得に成功しました 裏大会ID: " . $new_tournament_id, __FUNCTION__, basename( __FILE__ ) );
...
function insertNewTeam( $p_tournamentId, $p_teamId, $p_newTournamentId ) {
	...
	global $g_Log;
	$g_Log->debug( "チーム情報登録処理 tournament_id: $p_tournamentId, team_id: $p_teamId, new_tournament_id: $p_newTournamentId", __FUNCTION__, basename( __FILE__ ) );
...
function insertNextGame( $p_tournamentId, $p_gameBlock, $p_gameCount, $p_teamId ){
	...
	global $g_Log;
	$g_Log->debug( "次回戦追加処理 tournament_id: $p_tournamentId, game_block: $p_gameBlock, game_count: $p_gameCount, team_id: $p_teamId", __FUNCTION__, basename( __FILE__ ) );
	...
	try {
		...
		if( $stmt === 0 ){
			...
			$g_Log->debug( "裏大会追加処理 件数: $stmt", __FUNCTION__, basename( __FILE__ ) );
...
function insertUraTournament( $p_tournamentId ){
	...
	global $g_Log;
	$g_Log->debug( "裏大会内容追加処理 tournament_id: $p_tournamentId", __FUNCTION__, basename( __FILE__ ) );
	...
	try {
		...
		foreach( $dataTable as $row ){
			...
			$g_Log->debug( "裏大会ID取得 : tournament_id={$new_tournament_id} 件", __FUNCTION__, basename( __FILE__ ) );
			return $new_tournament_id;
		}

		$g_Log->debug( "裏大会検索 : 件数=" . count( $dataTable ) . " 件", __FUNCTION__, basename( __FILE__ ) );
		...
		$_SESSION[ 'RES_ERR_MSG' ] = '裏大会内容の追加に失敗しました';
		$g_Log->error( $_SESSION[ 'RES_ERR_MSG' ] . " : " . htmlspecialchars( $e->getMessage() ), __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
...
<?php
	// 試合登録の読み込み
	$filename = __DIR__ . DIRECTORY_SEPARATOR . "article_gameschedule.php";
	if ( file_exists( $filename ) ){
		require_once( $filename );
	}
?>
