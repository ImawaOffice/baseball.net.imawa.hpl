...
font-size:25px;
			...
			<div id="dtn"  style="display:none;">
				...
				<textarea id="teamList" cols="50" rows="16">
					<?php 
						foreach ( getTeamList( $_SESSION[ 'RES_TOURNAMENT_ID' ], 1, 100 ) as $team ){
				...
				<textarea id="game1List" cols="50" rows="16">
					<?php 
						foreach ( getGameResult( $_SESSION[ 'RES_TOURNAMENT_ID' ], 0 ) as $game ){
				...
				<textarea id="game2List" cols="50" rows="16">
					<?php 
						foreach ( getGameResult( $_SESSION[ 'RES_TOURNAMENT_ID' ], 1 ) as $game ){
							echo htmlspecialchars( $game[ 'game_block' ], ENT_QUOTES, 'UTF-8' ) . ",";
							echo htmlspecialchars( $game[ 'game_count' ], ENT_QUOTES, 'UTF-8' ) . ",";
							echo htmlspecialchars( $game[ 'game_count' ], ENT_QUOTES, 'UTF-8' ) . ",";
							echo htmlspecialchars( $game[ 'team1_name' ], ENT_QUOTES, 'UTF-8' ) . ",";
			...
			<div id="class0CanvasWrapper"  style="width: 100%; overflow: auto;" >
<!--			<div id="class0CanvasWrapper"  style="width:800px;height:600px" >	-->
			...
			<div id="class1CanvasWrapper"  style="width: 100%; overflow: auto;" >
<!--			<div id="class1CanvasWrapper"  style="width:800px;height:600px" >	-->
			...
			<div id="class2CanvasWrapper"  style="width: 100%; overflow: auto;" >
...
function getGameList( $gameListId = "game1List" ){
	...
	let gameArray = gameListValue.replace(/\t/g, "").split( "\n" );
	let round1Count = 0;
	let newGameList = new Array();
///	newGameList[ 0 ] = ",,,-,-,-,,,,0,,";
	console.log( "getGameList : " + $gameListId + " Data : " + gameArray );
	let newLine = 0;
	for (let i = 0; i < gameArray.length; i++) {
		
		let gameRow = gameArray[ i ].split( "," );
		...
		if( gameRow[ 0 ] >= 1 ){
			newLine++;
			newGameList[ newLine ] = gameArray[ i ];
		}
	}
	console.log( "getGameList: round1Count : " + round1Count );

	// 回戦数を取得
	gameRound = Math.log( round1Count - 1 ) / Math.log( 2 );
	gameToalTound = Math.ceil( gameRound ) + 1;

	console.log( "getGameList: gameRound : " + gameRound + " gameToalTound : " + gameToalTound );
	...
	for( let r = 1; r <= gameTotalRound; r++ ){
		...
		gameStartIndex += 1;
		console.log( `getGameList: gameStartIndex=${gameStartIndex}, gameEndIndex=${gameEndIndex}` );
...
function createCanvas( $canvasWrapperId, $canvasId, $canvasImageId, $gameListId, $width, $height ){
	...
	// 2Dコンテキストの取得
	let canvasContext = targetCanvas.getContext( '2d' );
	// フォント設定
