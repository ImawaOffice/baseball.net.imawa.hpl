...
function initProc() {
	...
	$sessionList = [
		...
		'verificationPeriod' => 0,	// 認証有効期限
		'password'           => '',	// パスワード
		'send_button'        => '送信',	// 送信ボタン
		'backto'             => 0		// 戻るボタン制御
	];
...
function requestProc() {
	...
	if( isset( $_POST[ 'view_state' ] ) ) {
		...
		$g_Log->debug( "SESSION : view_state = " . $_POST[ 'view_state' ], __FUNCTION__, basename( __FILE__ ) );
	}

	// 戻るボタンの取得
	if( isset( $_POST[ 'backto' ] ) ) {
		$_SESSION[ 'backto' ] = $_POST[ 'backto' ];
		$g_Log->debug( "SESSION : backto = " . $_POST[ 'backto' ], __FUNCTION__, basename( __FILE__ ) );
	}

	// メールアドレスの取得
	if( isset( $_POST[ 'email' ] ) ) {
		$_SESSION[ 'email' ] = trim( $_POST[ 'email' ] );
		$g_Log->debug( "SESSION : email = " . $_POST[ 'email' ], __FUNCTION__, basename( __FILE__ ) );
	}
	
	// 認証コードの取得
	if( isset( $_POST[ 'code' ] ) ) {
		$_SESSION[ 'code' ] = trim( $_POST[ 'code' ] );
		$g_Log->debug( "SESSION : code = " . $_POST[ 'code' ], __FUNCTION__, basename( __FILE__ ) );
	}
	
	// パスワードの取得
	if( isset( $_POST[ 'password' ] ) ) {
		$_SESSION[ 'password' ] = trim( $_POST[ 'password' ] );
		$g_Log->debug( "SESSION : password = " . $_POST[ 'password' ], __FUNCTION__, basename( __FILE__ ) );
	}

	// 前のステップに戻る
	$_SESSION[ 'view_state' ] = $_SESSION[ 'view_state' ] - $_SESSION[ 'backto' ];
	$g_Log->debug( "SESSION : view_state = " . $_SESSION[ 'view_state' ], __FUNCTION__, basename( __FILE__ ) );
...
function checkViewState() {
	...
	global $g_Log;
	$g_Log->debug( "表示ステータスチェック処理 : " . $_SESSION[ 'view_state' ], __FUNCTION__, basename( __FILE__ ) );
...
function checkMailAddress() {
	...
	global $g_Log;
	$g_Log->debug( "メールアドレスチェック処理 : " . $_SESSION[ 'email' ], __FUNCTION__, basename( __FILE__ ) );
...
function checkVerificationCode(){
	...
	global $g_Log;
	$g_Log->debug( "認証コードチェック処理 : " . $_SESSION[ 'code' ], __FUNCTION__, basename( __FILE__ ) );
...
function checkPassword() {
	...
	global $g_Log;
	$g_Log->debug( "パスワードチェック処理 : " . $_SESSION[ 'password' ], __FUNCTION__, basename( __FILE__ ) );
...
function setProc() {
	...
	if( $_SESSION[ "backto" ] > 0 ) {
		$g_Log->debug( "前のステップに戻るため以降の処理なし", __FUNCTION__, basename( __FILE__ ) );
		return true;
	}
	...
	if( $_SESSION[ 'view_state' ] == 2 ) {
		...
		$g_Log->debug( "サインイン画面に遷移します。", __FUNCTION__, basename( __FILE__ ) );
...
function setVerificationCode() {
	...
	global $g_Log;
	$g_Log->debug( "認証コード作成処理 : " . $_SESSION[ 'email' ], __FUNCTION__, basename( __FILE__ ) );
	...
	$g_Log->debug( "認証コードを作成しました : " . $randomCode, __FUNCTION__, basename( __FILE__ ) );
...
function setMailAddress() {
	...
	global $g_Log;
	$g_Log->debug( "メールアドレス登録処理 : " . $_SESSION[ 'email' ], __FUNCTION__, basename( __FILE__ ) );
	...
	<div class="container">
		...
		<div class="signup-container">
			<div class="signup-step">
				<ul>
					<li style="<?php echo $_SESSION[ 'view_style' ][ 0 ]; ?>">ステップ1 : メールアドレス入力・認証コード送信</li>
					<li style="<?php echo $_SESSION[ 'view_style' ][ 1 ]; ?>">ステップ2 : 認証コード入力・認証チェック</li>
					<li style="<?php echo $_SESSION[ 'view_style' ][ 2 ]; ?>">ステップ3 : パスワード設定・登録完了</li>
				</ul>
			</div>
			<!-- 1. メールアドレス入力・認証コード送信 -->
			<form method="post" action="./signup" novalidate >
				...
				<div class=form-group>
					...
					<?php $email_readonly = ( $_SESSION[ 'view_state' ] == 0 ) ? "" : "readonly"; ?>
					<?php $email_autofocus = ( $_SESSION[ 'view_state' ] == 0 ) ? "autofocus" : ""; ?>
					<input type="email" id="email" name="email" value="<?php echo isset( $_SESSION[ 'email' ] ) ? htmlspecialchars( $_SESSION[ 'email' ] ) : ''; ?>" <?php echo $email_readonly; ?> <?php echo $email_autofocus; ?> required>
				</div>

				<?php if( $_SESSION[ 'view_state' ] == 1 ) : ?>
					<?php $code_autofocus = ( $_SESSION[ 'view_state' ] == 1 ) ? "autofocus" : ""; ?>
					<div class=form-group>
						<label for="code">認証コード（6桁数字）</label>
						<input type="text" id="code" name="code" maxlength="6" pattern="[0-9]{6}" <?php echo $code_autofocus; ?> required>
					</div>
				<?php endif; ?>

				<?php if( $_SESSION[ 'view_state' ] == 2 ) : ?>
					<?php $password_autofocus = ( $_SESSION[ 'view_state' ] == 2 ) ? "autofocus" : ""; ?>
					<div class=form-group>
						<label for="password">パスワード設定（6文字以上）</label>
				...
				<?php if( $_SESSION[ 'view_state' ] != 0 ) : ?>
					<div class=form-group>
						<label></label>
						<button type="submit"name="backto" value="1">前のステップに戻る</button>
					</div>
				<?php endif; ?>
			</form>
		</div>
