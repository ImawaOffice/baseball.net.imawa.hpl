...
function requestPOST() {
	...
	// view_stateの取得（数値化・未設定時は0）
	$_SESSION['view_state'] = isset($_POST['view_state']) && is_numeric($_POST['view_state']) ? (int)$_POST['view_state'] : (isset($_SESSION['view_state']) ? (int)$_SESSION['view_state'] : 0);
	$g_Log->notice("SESSION : view_state = " . $_SESSION['view_state'], __FUNCTION__, basename(__FILE__));

	// 戻るボタンの取得（数値化・未設定時は0）
	$_SESSION['backto'] = isset($_POST['backto']) && is_numeric($_POST['backto']) ? (int)$_POST['backto'] : (isset($_SESSION['backto']) ? (int)$_SESSION['backto'] : 0);
	$g_Log->notice("SESSION : backto = " . $_SESSION['backto'], __FUNCTION__, basename(__FILE__));

	// メールアドレスの取得（サニタイズ・未設定時は空文字）
	$_SESSION['email'] = isset($_POST['email']) ? trim(filter_var($_POST['email'], FILTER_SANITIZE_EMAIL)) : (isset($_SESSION['email']) ? $_SESSION['email'] : '');
	$g_Log->notice("SESSION : email = " . $_SESSION['email'], __FUNCTION__, basename(__FILE__));

	// 認証コードの取得（サニタイズ・未設定時は空文字）
	$_SESSION['code'] = isset($_POST['code']) ? trim(htmlspecialchars($_POST['code'], ENT_QUOTES, 'UTF-8')) : (isset($_SESSION['code']) ? $_SESSION['code'] : '');
	$g_Log->notice("SESSION : code = " . $_SESSION['code'], __FUNCTION__, basename(__FILE__));

	// パスワードの取得（サニタイズ・未設定時は空文字）
	$_SESSION['password'] = isset($_POST['password']) ? trim(htmlspecialchars($_POST['password'], ENT_QUOTES, 'UTF-8')) : (isset($_SESSION['password']) ? $_SESSION['password'] : '');
	$g_Log->notice("SESSION : password = " . $_SESSION['password'], __FUNCTION__, basename(__FILE__));

	// 前のステップに戻る（未設定時は0）
	$_SESSION['view_state'] = (isset($_SESSION['view_state']) ? (int)$_SESSION['view_state'] : 0) - (isset($_SESSION['backto']) ? (int)$_SESSION['backto'] : 0);
	$g_Log->notice("SESSION : view_state = " . $_SESSION['view_state'], __FUNCTION__, basename(__FILE__));
...
function checkProc() {
	...
	// メールアドレス入力
	if( isset($_SESSION['view_state']) && $_SESSION['view_state'] == 0 ) {
		// メールアドレスのチェック
		if( ! checkMailAddress() ) {
			return false;
		}
	...
	// 認証コード入力
	if( isset($_SESSION['view_state']) && $_SESSION['view_state'] == 1 ) {
		// メールアドレスのチェック
	...
	// パスワード入力
	if( isset($_SESSION['view_state']) && $_SESSION['view_state'] == 2 ) {
		// パスワードのチェック
...
function checkViewState() {
	...
	global $g_Log;
	$view_state = isset($_SESSION['view_state']) ? $_SESSION['view_state'] : 0;
	$g_Log->notice( "表示ステータスチェック処理 : " . $view_state, __FUNCTION__, basename( __FILE__ ) );

	if( ! is_numeric( $view_state ) ) {
		$view_state = 0;
	}
	if( $view_state < 0 ) {
		$view_state = 0;
	...
	$_SESSION['view_state'] = $view_state;
	return true;
}
...
function checkMailAddress() {
	...
	$g_Log->notice( "メールアドレスチェック処理 : " . $email, __FUNCTION__, basename( __FILE__ ) );
...
function checkVerificationCode(){
	...
	global $g_Log;
	$inputCode = isset($_SESSION['code']) ? $_SESSION['code'] : '';
	$email = isset($_SESSION['email']) ? $_SESSION['email'] : '';
	$g_Log->notice( "認証コードチェック処理 : " . $inputCode, __FUNCTION__, basename( __FILE__ ) );
	$period = time() - ( 60 * 60 );
...
# パスワードチェック処理
# ==========================================================
function checkPasswordIndex() {

	global $g_Log;

	$password = isset($_SESSION['password']) ? $_SESSION['password'] : '';
	$g_Log->notice( "パスワードチェック処理 : " . $password, __FUNCTION__, basename( __FILE__ ) );
...
function setProc() {
	...
	if( isset($_SESSION["backto"]) && $_SESSION["backto"] > 0 ) {
...
function setMailAddress() {
	...
	$g_Log->notice( "メールアドレス登録処理 : " . $_SESSION[ 'email' ], __FUNCTION__, basename( __FILE__ ) );
	   ...
	   $email = isset($_SESSION['email']) ? $_SESSION['email'] : '';
	   $randomCode = isset($_SESSION['VerificationCode']) ? $_SESSION['VerificationCode'] : '';
	   $period = isset($_SESSION['verificationPeriod']) ? $_SESSION['verificationPeriod'] : '';

	   if ($email === '' || $randomCode === '' || $period === '') {
		   $_SESSION['error_msg'] = '内部エラー：認証コードまたは有効期限が未設定です。';
		   $g_Log->error($_SESSION['error_msg'] . " email={$email} code={$randomCode} period={$period}", __FUNCTION__, basename(__FILE__));
		   return false;
	   }
...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TVJ69Z2T"
...
<main class="page-main">
	<div class="container">
		...
		<div class="page-main-content">
			<img src="./images/baseball_top.jpg" alt="Baseball Image" class="top-image">
		</div>
	</div>
...
<?php
	// お知らせの読み込み
	$filename = __DIR__ . DIRECTORY_SEPARATOR . "article_information.php";
	if ( file_exists( $filename ) ){
		require_once( $filename );
	}
?>
...
<?php
	// 試合結果の読み込み
///	$filename = __DIR__ . DIRECTORY_SEPARATOR . "article_tournament-results.php";
///	if ( file_exists( $filename ) ){
///		require_once( $filename );
///	}
?>
...
<?php
	// 過去大会の読み込み
###	$filename = __DIR__ . DIRECTORY_SEPARATOR . "article_past-tournament.php";
###	if ( file_exists( $filename ) ){
###		require_once( $filename );
###	}
?>
...
<?php
	// スポンサーの読み込み
	$filename = __DIR__ . DIRECTORY_SEPARATOR . "article_sponsor.php";
	if ( file_exists( $filename ) ){
		require_once( $filename );
	}
?>
...
<?php
	// ソーシャルメディアの読み込み
	$filename = __DIR__ . DIRECTORY_SEPARATOR . "article_sns.php";
	if ( file_exists( $filename ) ){
		require_once( $filename );
	}
?>
