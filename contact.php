...
function requestProc() {
	...
	if( isset( $_POST[ 'token' ] ) ) {
		$g_Log->notice( "POST : token = " . $_POST[ 'token' ], __FUNCTION__, basename( __FILE__ ) );
	}
	
	// 名前の取得
	if( isset( $_POST[ 'name' ] ) ) {
		$g_Log->notice( "POST : name = " . $_POST[ 'name' ], __FUNCTION__, basename( __FILE__ ) );
	}
	
	// 電話番号の取得
	if( isset( $_POST[ 'phone' ] ) ) {
		$g_Log->notice( "POST : phone = " . $_POST[ 'phone' ], __FUNCTION__, basename( __FILE__ ) );
	}
	...
	if( isset( $_POST[ 'inquiry' ] ) ) {
		$g_Log->notice( "POST : inquiry = " . $_POST[ 'inquiry' ], __FUNCTION__, basename( __FILE__ ) );
	}

	// 同意の取得
	if( isset( $_POST[ 'agree' ] ) ) {
		$g_Log->notice( "POST : agree = " . $_POST[ 'agree' ], __FUNCTION__, basename( __FILE__ ) );
	}
...
function checkProc() {
	...
	if ( $name == '' ) {
		...
		$g_Log->warning( $_SESSION[ 'FORM_ERR_MSG' ] . " : " . $name, __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
	...
	if( $email != '' ){
		if ( ! filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
			$_SESSION[ 'FORM_ERR_MSG' ] = 'メールアドレスの形式が正しくありません';
			$g_Log->warning( "INVALID FORMAT : " . $_SESSION[ 'FORM_ERR_MSG' ] . " : " . $_SESSION[ 'email' ], __FUNCTION__, basename( __FILE__ ) );
			return false;
		}
	}

	if ( $inquiry == '' ) {
		$_SESSION[ 'FORM_ERR_MSG' ] = 'お問い合わせ内容を入力して下さい。';
		$g_Log->warning( $_SESSION[ 'FORM_ERR_MSG' ] . " : " . $inquiry, __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
...
function setInquiry() {
	...
	global $g_Log;
	$g_Log->notice( "問合せ内容登録処理", __FUNCTION__, basename( __FILE__ ) );
	...
	try {
		...
		$_SESSION[ 'FORM_ERR_MSG' ] = '問合せ内容の登録処理でエラーが発生しました';
...
function sendMail() {
	...
	try {
		...
		if( count( $dataTable ) == 0 ){
			$_SESSION[ 'FORM_ERR_MSG' ] = 'メールアドレスが登録されていません。';
			$g_Log->warning( $_SESSION[ 'FORM_ERR_MSG' ] . " : " . $email, __FUNCTION__, basename( __FILE__ ) );
			return false;
		}
	...
	if ( $mailer->sendMail( $toAddress, $subject, $message, $ccAddress, $bccAddress ) ) {
		$g_Log->notice( "問合せ内容を送信しました", __FUNCTION__, basename( __FILE__ ) );
	} else {
		$_SESSION[ 'FORM_ERR_MSG' ] = '問合せ内容の送信に失敗しました。';
		$g_Log->error( $_SESSION[ 'FORM_ERR_MSG' ] . " : " . $email, __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
	
	$g_Log->notice( "メール送信処理が完了しました : " . $email, __FUNCTION__, basename( __FILE__ ) );
