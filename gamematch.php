...
function requestProc() {
	...
	if( isset( $_POST[ 'game_class' ] ) ) {
		$g_Log->notice( "POST : game_class = " . $_POST[ 'game_class' ], __FUNCTION__, basename( __FILE__ ) );
	}

	// チームIDの取得
	if( isset( $_POST[ 'team1_id' ] ) ) {
		$g_Log->notice( "POST : team1_id = " . $_POST[ 'team1_id' ], __FUNCTION__, basename( __FILE__ ) );
	}

	// チームIDの取得
	if( isset( $_POST[ 'team2_id' ] ) ) {
		$g_Log->notice( "POST : team2_id = " . $_POST[ 'team2_id' ], __FUNCTION__, basename( __FILE__ ) );
	}

	// チーム名の取得
	if( isset( $_POST[ 'team1_name' ] ) ) {
		$g_Log->notice( "POST : team1_name = " . $_POST[ 'team1_name' ], __FUNCTION__, basename( __FILE__ ) );
	}

	// チーム名の取得
	if( isset( $_POST[ 'team2_name' ] ) ) {
		$g_Log->notice( "POST : team2_name = " . $_POST[ 'team2_name' ], __FUNCTION__, basename( __FILE__ ) );
	}
...
function checkDelete() {
	...
	if( ! checkRequest( 'game_block', 'POST' ) ) {
		$_SESSION[ 'FORM_ERR_MSG' ] = "試合ブロックエラー";
		$g_Log->notice( "試合ブロックのチェックに失敗しました", __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
...
function checkForm(){
	...
	if( ! checkRequest( 'game_class', 'POST' ) ) {
		$_SESSION[ 'FORM_ERR_MSG' ] = "試合分類エラー";
		$g_Log->notice( "試合分類のチェックに失敗しました", __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
	...
	if( ! checkRequest( 'team1_id', 'POST' ) ) {
		$_SESSION[ 'FORM_ERR_MSG' ] = "チーム名１エラー";
		$g_Log->notice( "チーム名１のチェックに失敗しました", __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
	...
	if( ! checkRequest( 'team2_id', 'POST' ) ) {
		$_SESSION[ 'FORM_ERR_MSG' ] = "チーム名２エラー";
		$g_Log->notice( "チーム名２のチェックに失敗しました", __FUNCTION__, basename( __FILE__ ) );
		return false;
	}
